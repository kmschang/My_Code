...
def run_tidy(args, tmpdir, build_path, queue, write_to_file_queue):
    ...
    while True:
        ...
        stdout = subprocess.check_output(invocation)
        if args.output_file is not None:
            write_to_file_queue.put(stdout)
        queue.task_done()


# based on https://stackoverflow.com/a/11984760
class WriteToFileThread(threading.Thread):
...
def main():
    ...
    parser.add_argument(
        "-o", dest="output_file", default="clang-tidy.out", help="Save warnings to file"
    )
    parser.add_argument(
    ...
    try:
        ...
        write_to_file_queue = queue.Queue()
        for _ in range(max_task):
            t = threading.Thread(
                target=run_tidy,
                args=(args, tmpdir, build_path, task_queue, write_to_file_queue),
            )
            t.daemon = True
            t.start()
        write_to_file_thread = WriteToFileThread(args.output_file, write_to_file_queue)
